...
- (void)layoutSubviews {
    ...
    self.tableView.frame = CGRectMake(margin, margin, self.frame.size.width - 2 * margin, self.frame.size.height - 2 * margin);
}
...
- (void)setupTableView {
    ...
    [tableView registerClass:[FTFormTableViewCell class] forCellReuseIdentifier:@"reuseIdentifier"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        cell = [[FTFormTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"FTFormTableViewCell"];
    }
...
{
    FTFormModel *formModel = self.formModelArray[indexPath.row];
    CGFloat padding = tableView.frame.size.width - formModel.separateLineWidth;
